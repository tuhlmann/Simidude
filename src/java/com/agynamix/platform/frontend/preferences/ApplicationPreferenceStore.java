 ...
 */
public class ApplicationPreferenceStore implements IPersistentPreferenceStore {
    /**
     ...
     */
    public ApplicationPreferenceStore(IPreferenceConfigAdapter configAdapter) {
      this.configAdapter = configAdapter;
      defaultProperties = new Properties();
      this.loadDefaults(configAdapter.loadDefaultProperties());
      properties = new Properties(defaultProperties);
      this.load(configAdapter.loadProperties());      
    }
    /*
    ...
    protected void load(Properties in) {
      for (Iterator it = in.keySet().iterator(); it.hasNext();) {
        String key = (String) it.next();
        properties.setProperty(key, in.getProperty(key));
      }
      dirty = false;
    }

    protected void loadDefaults(Properties in) {
      for (Iterator it = in.keySet().iterator(); it.hasNext();) {
        String key = (String) it.next();
        defaultProperties.setProperty(key, in.getProperty(key));
      }
    }
     ...
     */
    public void save() {
      configAdapter.saveProperties(properties);
      dirty = false;
    }
    /*
     * (non-Javadoc) Method declared on IPreferenceStore.
